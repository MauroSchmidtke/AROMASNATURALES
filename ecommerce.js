const contenedorProductos = document.getElementById('contenedor-productos'); //En este contenedor voy a agregar mis productos en stock
let botonComprar = document.getElementById("comprar"); // Voy a escuchar el evento click en el boton "Realizar Compra"
...
//Agrego el stock de mis productos guardado en el archivo .json
...
//Vuelco mi stock de productos en el HTML (div 'contenedor-productos')
...
stockProductos.forEach((producto) => {
    ...
    `
    //Notar que cada boton que se crea de "Agregar" esta compuesto con el ID para hacerlo especifico a cada producto
    contenedorProductos.appendChild(div);


    //Escucho cada click en los botones "Agregar" y recopilo su informacion
...
function borrarElemento(event) {
    ...
    Swal.fire({
        title: '"Esta seguro que quiere quitar este productor del carrito?"',
        ...
        if (result.isConfirmed) 
        {
            ...
            Toastify({
                text: "Se elimino correctamente el producto del carrito",
                duration: 2000,
                gravity: 'bottom'
...
//Escucho el click del boton "Comprar"
    ...
    if (total === 0){
        ...
        Swal.fire({
            title: '!!',
            text: 'Gracias por su compra! Gastaste: $'+total,
            icon: 'success',
    ...
    for (let i = 0; i < itemID.length; i++) {
        ...
        //console.log(IMG);
        //let longitud = IMG.length;
        //let productoIMG = "./"+IMG.substring(88,longitud);
...
botonUltimaCompra.addEventListener('click', () => {
    ...
    if(almacenados){
        ...
        Swal.fire({
            title: 'Error!',
            text: 'No hay registro de compra Anterior',
            icon: 'error',
